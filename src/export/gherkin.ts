...
export function exportGherkin(args: { key: string; output?: string; cwd?: string }) {
  ...
  const rel = relative(resolve(config.root), outputPath);
  if (rel.startsWith("..") || isAbsolute(rel)) {
    throw new VspecError("INVALID_ARGUMENT", `Path traversal detected: output path "${output}" resolves outside the project root.`);
  }
