...
function validateUseCase(args: {
  ...
  validateStepsAndExtensions(findings, useCase, actors, glossary, location);
  validateFormatAndLimits(findings, useCase, location);
...
function validateStepsAndExtensions(
  findings: Finding[],
  useCase: ParsedUseCase,
  actors: EntityIndex,
  glossary: Glossary,
...
function validateFormatAndLimits(
  findings: Finding[],
  useCase: ParsedUseCase,
  location: string,
) {
