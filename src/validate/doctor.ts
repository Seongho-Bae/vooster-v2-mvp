...
function readEntityIndex(root: string, dir: string): EntityIndex {
  ...
  let lazyByDisplay: Map<string, string> | null = null;
  const files = walkFiles(join(root, dir), (path) => path.endsWith(".md"));
        ...
        for (const file of files) {
          ...
          try {
            ...
            if (
              ...
              lazyByDisplay.set(data.display_name.trim().toLowerCase(), slug);
            }
          } catch {
