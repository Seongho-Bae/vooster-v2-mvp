...
export function buildProgram(): Command {
      ...
      runCommand(
        ...
        (result) => ({
          ...
          suggestedNextActions: [
            {
              command: "vspec actor create --name developer",
              reason: "Add the primary actor.",
            },
            {
              command:
                'vspec usecase create --title "..." --primary-actor developer',
              reason: "Create the first use case.",
            },
          ],
        }),
      ),
  ...
  program
    .command("ai-guide")
    .description("Print an end-to-end authoring guide")
    .action(() => {
      outputSuccess({
        data: { guide: aiGuideText() },
        suggestedNextActions: [
          { command: "vspec init --key VSPEC", reason: "Start the workflow." },
        ],
      });
    });
      ...
      outputSuccess({
        ...
        suggestedNextActions: [
          ...result.promotable.map((key) => ({
            ...
            reason:
              "All required sections are present — promote from BRIEF/CASUAL to FULLY_DRESSED.",
          })),
          {
            command: "vspec export gherkin <KEY>",
            reason: "Export validated use cases.",
          },
        ],
  ...
  const usecase = program
    .command("usecase")
    .description("Create, list, and show use cases");
  usecase
    .command("create")
      ...
      (options: {
        ...
        runCommand(
          ...
          (created) => ({
            ...
            suggestedNextActions: [
              {
                command: `vspec usecase apply ${created.key} --section main-success`,
                reason:
                  "Author the main success steps (pipe the numbered list via stdin).",
              },
              {
                command: `vspec doctor ${created.key}`,
  ...
  usecase
    ...
    .requiredOption(
      "--field <name>",
      "title|level|format|status|priority|scope|frequency",
    )
    .requiredOption(
      "--value <value>",
      "new value (validated against the field)",
    )
    .action((key: string, options: { field: string; value: string }) =>
      runCommand(
        () => setUseCaseField({ key, ...options }),
        (result) => mutationPayload(result, key),
      ),
    );
  usecase
    .command("apply")
    .description(
      "Replace the body (or one --section) from content piped via stdin",
    )
    .argument("<key>", "use-case key")
    .option(
      "--section <name>",
      `one of: ${BODY_SECTIONS.join("|")} (omit to replace the whole body)`,
    )
  ...
  const actor = program
    .command("actor")
    .description("Create, list, and show actors");
  actor
    .command("create")
    ...
    .option(
      "--human",
      "force the actor human (default: human for primary, non-human otherwise)",
    )
      ...
      (options: {
        ...
        runCommand(
          ...
          (result) =>
            entityPayload(
              result,
              'vspec usecase create --title "..." --primary-actor ' +
                result.name,
            ),
        ),
    );
  actor.command("list").action(() =>
    runCommand(
      ...
      (data) => ({
        data,
        suggestedNextActions: [{ command: "vspec actor show <name>" }],
      }),
    ),
  );
  actor
      ...
      runCommand(
        ...
        (data) => ({
          ...
          suggestedNextActions: [
            {
              command:
                'vspec usecase create --title "..." --primary-actor ' + name,
            },
          ],
        }),
      ),
  ...
  actor
    ...
    .action((name: string, options: { field: string; value: string }) =>
      runCommand(
        () => setActorField({ name, ...options }),
        (result) => entityMutationPayload(result),
      ),
    );
  actor
    .command("apply")
    .description(
      "Replace the actor description body from content piped via stdin",
    )
    .argument("<name>")
    .action((name: string) =>
      runCommand(
        () => applyActorBody({ name, body: readStdin() }),
        (result) => entityMutationPayload(result),
      ),
    );

  const stakeholder = program
    .command("stakeholder")
    .description("Create, list, and show stakeholders");
  stakeholder
    .command("create")
    .requiredOption("--name <name>")
    .option("--display-name <displayName>")
    ...
    .action((options: { name: string; displayName?: string; type?: string }) =>
      runCommand(
        () => createStakeholder(options),
        (result) =>
          entityPayload(
            result,
            "vspec usecase apply <KEY> --section stakeholders",
          ),
      ),
    );
  stakeholder.command("list").action(() =>
    runCommand(
      async () => await listStakeholders({}),
      (data) => ({
        data,
        suggestedNextActions: [{ command: "vspec stakeholder show <name>" }],
      }),
    ),
  );
  stakeholder
    ...
    .action((name: string) =>
      runCommand(
        ...
        (data) => ({
          data,
          suggestedNextActions: [
            { command: "vspec usecase apply <KEY> --section stakeholders" },
          ],
        }),
      ),
    );
  stakeholder
    ...
    .action((name: string, options: { field: string; value: string }) =>
      runCommand(
        () => setStakeholderField({ name, ...options }),
        (result) => entityMutationPayload(result),
      ),
    );
  stakeholder
    .command("apply")
    .description(
      "Replace the stakeholder description body from content piped via stdin",
    )
    .argument("<name>")
    .action((name: string) =>
      runCommand(
        () => applyStakeholderBody({ name, body: readStdin() }),
        (result) => entityMutationPayload(result),
      ),
    );
  ...
  const goal = program
    .command("goal")
    .description("Create, list, show, promote, and reject goals");
  goal
    .command("create")
    ...
    .action(
      (options: {
        ...
        runCommand(
          () => createGoal(options),
          (result) => entityPayload(result, `vspec goal promote ${result.id}`),
        ),
    );
  goal
    .command("list")
    ...
    .action((options: { actor?: string; status?: string }) =>
      runCommand(
        ...
        (data) => ({
          data,
          suggestedNextActions: [{ command: "vspec goal show <G-NNN>" }],
        }),
      ),
    );
  goal
    ...
    .action((id: string) =>
      runCommand(
        ...
        (data) => ({
          data,
          suggestedNextActions: [{ command: `vspec goal promote ${id}` }],
        }),
      ),
    );
  goal
  ...
  goal
    ...
    .action((id: string) =>
      runCommand(
        ...
        (data) => ({
          data,
          suggestedNextActions: [{ command: "vspec goal list" }],
        }),
      ),
    );
    ...
    .action((key: string, options: { output?: string }) =>
      runCommand(
        ...
        (result) => ({
          ...
          suggestedNextActions: [
            {
              command: `git add ${result.path}`,
              reason: "Stage the exported feature when ready.",
            },
          ],
        }),
      ),
...
function suggestDoctorActions(findings: { rule: string; message: string }[]) {
  ...
  for (const finding of findings) {
    if (/actor .+? does not exist/i.test(finding.message)) {
      add(
        "vspec actor list",
        "Reference an existing actor by its slug (not its display name), or create it.",
      );
    }
    if (/stakeholder .+? does not exist/i.test(finding.message)) {
      add(
        "vspec stakeholder list",
        "Reference an existing stakeholder by its slug (not its display name), or create it.",
      );
    }
  }
...
async function runCommand<T>(
  ...
  try {
    ...
    outputSuccess(payload(result as Awaited<T>));
  } catch (error) {
...
function entityPayload<T extends { path: string }>(data: T, next: string) {
  return {
    ...
    suggestedNextActions: [{ command: next, reason: "Continue authoring." }],
  };
}
...
function mutationPayload<T extends { path: string }>(data: T, key: string) {
  return {
    ...
    affectedFiles: [{ path: data.path }],
    suggestedNextActions: [
      { command: `vspec doctor ${key}`, reason: "Validate after the edit." },
    ],
  };
}
...
function entityMutationPayload<T extends { path: string }>(data: T) {
  return {
    ...
    affectedFiles: [{ path: data.path }],
    suggestedNextActions: [
      { command: "vspec doctor", reason: "Re-validate specs after the edit." },
    ],
  };
}
...
function applyPayload<T extends { path: string }>(
  data: T,
  key: string,
  wholeBody: boolean,
) {
  ...
  const actions = [
    {
      ...
      reason:
        errors.length > 0
          ? `${errors.length} validation error(s) to fix.`
  ...
  if (wholeBody) {
    actions.push({
      ...
      reason:
        "For later edits, replace one --section at a time — fewer tokens and no risk of clobbering other sections.",
    });
  }
  ...
  return {
    ...
    warnings: findings.map((finding) => ({
      message: `${finding.level}: ${finding.message}`,
    })),
...
export async function run(argv: string[] = process.argv): Promise<void> {
  ...
  try {
    ...
    const raw = (err.message ?? "Invalid command invocation.").replace(
      /^error:\s*/i,
      "",
    );
