...
export function createActor(args: {
  ...
  writeFileSync(
    path,
    stringifyFrontmatter(
      orderActorFrontmatter(fm),
      `${fm.display_name} actor.\n`,
    ),
  );
...
export function listActors(args: { cwd?: string }) {
  const { root } = mustConfig(args.cwd);
  return walkFiles(join(root, "specs/actors"), (path) =>
    path.endsWith(".md"),
  ).map((path) => ({
...
export function showActor(args: { name: string; cwd?: string }) {
  ...
  return {
    frontmatter: parseActorFrontmatter(
      parseMatter(readFileSync(path, "utf8")).data,
    ),
    path: relativePath(path, root),
  };
}

export function setActorField(args: {
  name: string;
  field: string;
  value: string;
  cwd?: string;
  ...
  writeFileSync(
    path,
    stringifyFrontmatter(orderActorFrontmatter(fm), parsed.content),
  );
...
export function applyActorBody(args: {
  name: string;
  body: string;
  cwd?: string;
}) {
...
export function createStakeholder(args: {
  name: string;
  displayName?: string;
  type?: string;
  cwd?: string;
  ...
  if (!name)
    throw new VspecError(
      "INVALID_ARGUMENT",
      `stakeholder name "${args.name}" must be an ASCII slug like "user" — identifiers stay English even when the spec is Korean.`,
    );
  ...
  writeFileSync(
    path,
    stringifyFrontmatter(
      orderStakeholderFrontmatter(fm),
      `${fm.display_name} stakeholder.\n`,
    ),
  );
  return { name, path: relativePath(path, root) };
}
...
export function listStakeholders(args: { cwd?: string }) {
  const { root } = mustConfig(args.cwd);
  return walkFiles(join(root, "specs/stakeholders"), (path) =>
    path.endsWith(".md"),
  ).map((path) => ({
    ...parseStakeholderFrontmatter(
      parseMatter(readFileSync(path, "utf8")).data,
...
export function showStakeholder(args: { name: string; cwd?: string }) {
  ...
  return {
    frontmatter: parseStakeholderFrontmatter(
      parseMatter(readFileSync(path, "utf8")).data,
    ),
    path: relativePath(path, root),
  };
...
export function setStakeholderField(args: {
  name: string;
  field: string;
  value: string;
  cwd?: string;
  ...
  writeFileSync(
    path,
    stringifyFrontmatter(orderStakeholderFrontmatter(fm), parsed.content),
  );
  return { name: fm.name, path: relativePath(path, root) };
}

export function applyStakeholderBody(args: {
  name: string;
  body: string;
  cwd?: string;
}) {
  ...
  writeFileSync(
    path,
    stringifyFrontmatter(orderStakeholderFrontmatter(fm), body),
  );
  return { name: fm.name, path: relativePath(path, root) };
}
...
export function createGoal(args: {
  ...
  writeFileSync(
    path,
    stringifyFrontmatter(orderGoalFrontmatter(fm), `${args.description}\n`),
  );
  return { id, path: relativePath(path, root) };
}

export function listGoals(args: {
  actor?: string;
  status?: string;
  cwd?: string;
}) {
  ...
  return walkFiles(join(root, "specs/goals"), (path) => path.endsWith(".md"))
    .map((path) => ({
      path,
      frontmatter: parseGoalFrontmatter(
        parseMatter(readFileSync(path, "utf8")).data,
      ),
    }))
    .filter(
      ({ frontmatter }) =>
        !args.actor || frontmatter.actor === slugify(args.actor),
    )
    .filter(
      ({ frontmatter }) =>
        !args.status || frontmatter.status === args.status.toUpperCase(),
    )
...
export function promoteGoal(args: { id: string; cwd?: string }) {
  ...
  const created = createUseCase({
    cwd: args.cwd,
    title:
      readFileSync(join(mustConfig(args.cwd).root, goal.path), "utf8")
        .split("---")
        .pop()
        ?.trim() || goal.frontmatter.id,
...
function findGoalFile(root: string, id: string): string | null {
  return (
    walkFiles(join(root, "specs/goals"), (path) => path.endsWith(".md")).find(
      (path) => basename(path).startsWith(`${id}-`),
    ) ?? null
  );
}
...
function parseStakeholderType(
  value: string,
): StakeholderFrontmatter["stakeholder_type"] {
  const normalized = value.toUpperCase();
  if (
    normalized === "INTERNAL" ||
    normalized === "EXTERNAL" ||
...
function validatedActorType(raw: string): ActorFrontmatter["actor_type"] {
  ...
  throw new VspecError(
    "INVALID_ARGUMENT",
    `Invalid actor type "${raw}". Use one of: primary, supporting, offstage.`,
  );
}

function validatedStakeholderType(
  raw: string,
): StakeholderFrontmatter["stakeholder_type"] {
  const normalized = raw.toUpperCase();
  if (
  ...
  throw new VspecError(
    "INVALID_ARGUMENT",
    `Invalid stakeholder type "${raw}". Use one of: internal, external, regulatory.`,
  );
}
...
function validatedBool(field: string, raw: string): boolean {
  ...
  throw new VspecError(
    "INVALID_ARGUMENT",
    `Invalid ${field} "${raw}". Use true or false.`,
  );
}
