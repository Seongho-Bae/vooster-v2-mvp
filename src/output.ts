  ...
  console.log(
    JSON.stringify(
      buildOkEnvelope({
        ...
        affectedFiles:
          payload.affectedFiles ?? inferAffectedFiles(payload.data),
...
export function errorInfo(error: unknown): {
  ...
  if (error instanceof VspecError) {
    return {
      ...
      actions: error.actions ?? defaultActions(error.code),
    };
  }
      ...
      actions: [
        {
          ...
          reason:
            "Review the required frontmatter fields and valid enum values.",
        },
      ],
    };
  }
...
function inferAffectedFiles(data: unknown): { path: string }[] {
  if (data && typeof data === "object") {
    ...
    if (Array.isArray(record.affectedFiles))
      return record.affectedFiles.map((path) => ({ path: String(path) }));
...
function defaultActions(code: string): AgentAction[] {
  if (code === "KEY_NOT_FOUND")
    return [{ command: "vspec usecase list", reason: "See available keys." }];
  return [
    { command: "vspec ai-guide", reason: "See the end-to-end workflow." },
  ];
}
